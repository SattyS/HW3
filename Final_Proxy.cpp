...
void cacheClear(string hash, string wp){
    ...
    cout << "--------------------------------------------------" << endl;
    remove(hash.c_str());
    cout << "File Deleted " << hash << " " << getAddress(wp) << endl; 
    cout << "--------------------------------------------------" << endl;
}
...
void printList(){
	...
	//sleep(2);
	cout << "-------------------------------------------------" << endl;
        std::cout<<"Webpage " << wPTemp.webPageName << endl;
	cout << "Filename " << wPTemp.hash << endl;
	cout << "Last modified " << wPTemp.last_modified << endl;
	cout << "Expires " << wPTemp.expire_Time << endl;
	cout << "------------------------------------------------" << endl;
...
int lRU(string text, int connfd){
	...
	cout << "In LRU Last modified " << wPObj.last_modified << endl;
        wPObj.expire_Time = retS[1];
	cout << "In LRU expire time " << wPObj.expire_Time << endl;
        wPObj.webPageName = getAddress(text);
	cout << "WEBPAGE " << text << endl;
        wPObj.hash = hash;
        fileWrite(hash,text);
        ...
        {
            wPObj = *list_iter;
            if (wPObj.webPageName.compare(getAddress(text)) == 0){
                found = true;
                break;
            }
            i++;
        ...
        if(found){
            ...
            if(result == -1){
            	...
            	cout << "Last modified result " << result << endl;
		if(result == 0){
            ...
            wP wPObjNew;
            wPObjNew.webPageName = getAddress(text);
            wPObjNew.hash = hash;
                        ...
                        if ((readbytes = read(connfd,buf,sizeof buf)) > 0) {
                            ...
                            if(!checkPacket(buf)){
                                ...
                                if(lRURet == 2){
                                    ...
                                    string fileHash = hash_str(getAddress(sBuf).c_str());
