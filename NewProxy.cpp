...
int fileWrite(string hash,string fileName){    
    ...
    */
    ...
    std::size_t httpPos = fileName.find("http://");
    string fullHostAddr = fileName.substr(httpPos + 8, fileName.length());
    ...
    else
    {
        ...
        string getMessage = fileName.substr(0,httpPos + 7);
        cout << "GET message " << getMessage << endl;
        strcpy(Sbuf,getMessage.c_str());
        strcat(Sbuf,"\n\n");
